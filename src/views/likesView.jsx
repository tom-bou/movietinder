...
function LikedMoviesView(props) {
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-screen" style={{ background: 'linear-gradient(to bottom, #150629 60%, #1C0A34, #5A2960)' }}>
          <h1 className="text-3xl font-thin font-sans mt-8" style={{ color: '#FF7272', textShadow: '0px 0px 4px #FF3131' }}>Your Likes</h1>
          <h2 className="text-3xl font-thin font-sans mt-1 mr-10" style={{ color: '#FF7272', textShadow: '0px 0px 4px #FF3131' }}>{numberOflikes}</h2>
          <img src={likebutton} alt="Heart icon" className="absolute top-12 w-11 mt-5 ml-7" />
          <h1 className="text-3xl font-thin font-sans absolute top-5 right-32 w-11 mt-5" style={{ color: '#FFE370', textShadow: '0px 0px 4px #FFE370' }}>Swipe</h1>
          <img src={fire} alt="Fire icon" className="absolute right-14 top-7 w-10" />
      ...
      function renderMovie(movie) {
        return (
          <div key={movie.id} onClick={() => movieClickACB(movie)} className="mb-2">
            ...
            <img src={movie.picture} alt={movie.title} className="rounded-lg shadow-lg" style={{ filter: 'drop-shadow(0 0 0.75rem #D300FE)' }} width="190" />
          </div>
        );
      }
    }
