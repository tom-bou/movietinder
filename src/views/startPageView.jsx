...
function StartPageView({ firebaseModel }) {
  ...
  if (currentSessionId) {
    return (
      ...
      >
        <div
          className="mt-10 p-6 rounded-lg shadow-lg bg-white max-w-md animate-fade-down w-full border-4 border-[#4AADAB] bg-[#162534] z-10 shadow-lg"
  ...
  return (
    <div
      className="snap-y snap-mandatory overflow-scroll h-screen w-screen bg-fixed"
      style={{
        background:
          "linear-gradient(to bottom, #150629 30%, #1C0A34 40%, #8E4599)",
      }}
    >
      {/* SCROLL SECTION 1*/}
      <div className=" snap-start h-screen grid grid-rows-2 grid-cols-1 justify-items-center items-center block">
        <nav style={{width: "100%"}} class="font-sans rounded-sm z-10 bg-[#150629cb] fixed  top-0 flex justify-between items-center py-2 px-8">
        ...
        <img
          className="pt-16 md:pt-96 w-2/3 shadow-inner animate-fade animate-delay-[400ms]"
          style={{ filter: "drop-shadow(0 0 0.4rem #C772ED)" }}
          src={biglogo}
          alt="Logo icon"
        />
        <h1
          className="text-3xl flex col font-thin font-sans md:pt-56 tracking-wide justify-center text-center animate-fade-right animate-delay-[900ms]"
          style={{ color: "#FFFFFF", textShadow: "0px 0px 3px #FFFFFF" }}
        >
          Swipe, Like, Save Time On Your Movie Night!
        </h1>
        <img
          className="animate-bounce animate-duration-[2500ms] md:w-20 md:pb-12 md:pt-8"
          style={{ filter: "drop-shadow(0 0 0.4rem #C772ED)" }}
          src={arrowdown}
          alt="Arrow down"
        />
